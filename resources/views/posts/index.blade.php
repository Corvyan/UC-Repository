@extends('layouts.app')

@section('content')
    <div class="flex justify-center">
        <div class="w-8/12 bg-white p-6 rounded-lg">
            @auth
                @if(auth()->user()->role == "3")
                    <p class="mb-1">Petunjuk Pengiriman File Tugas Akhir : </p>   
                    <a href="{{ asset('pdf/petunjuk.pdf') }}" class="text-blue-500">Link Petunjuk</a>
                    <br>
                    <br>
                    <p class="mb-1">File Watermark : </p>   
                    <a href="" class="text-blue-500">Watermark</a>
        
                    <br> 
                    <br>
                    
                    <form action="{{ route('posts') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-4">
                            <label for="body" class="sr-only">Body</label>
                            <textarea name="body" id="body" cols="30" rows="4" class="bg-gray-100
                            border-2 w-full p-4 rounded-lg @error('body') border-red-500 @enderror"
                            placeholder="Judul Tugas Akhir"></textarea>
                            
                        </div>

                        @error('body')
                            <div class="text-red-500 mt-2 text-sm">
                                {{ $message }}
                            </div>
                        @enderror
                        
                        <input type="file" class="mt-4" name="thesis"> <br>

                        @error('thesis')
                        <div class="text-red-500 mt-2 text-sm">
                            {{ $message }}
                        </div>
                        @enderror
                        
                        <div>
                            <button type="submit" class="bg-blue-500 mt-4 mb-4 text-white px-4 py-2 rounded
                            font-medium">Upload</button>
                        </div>
                    </form>
                @endif
            @endauth
 
            @if ($posts->count())
                @foreach ($posts as $post)
                    <x-post :post="$post" />
                @endforeach

                {{ $posts->links() }}
            @else
                <p>There are no posts</p>
            @endif
            
        </div>

    </div>
@endsection